...
- **Cookie**：
  - 存储在浏览器中，服务器可以通过 HTTP 请求发送和接收 Cookie。
  - 每次请求都会自动发送与当前域名匹配的 Cookie。
- **sessionStorage**：
  - 存储在浏览器的内存中，仅在当前浏览器窗口或标签页的会话中有效。
  - 不会随请求发送到服务器。
- **localStorage**：
  - 存储在浏览器中，数据在浏览器关闭后仍然存在，直到手动删除。
  - 不会随请求发送到服务器。

#### 2. **有效期**

- **Cookie**：
  - 有效期可以设定，超过设定时间后会被删除。如果没有设定，Cookie 会在浏览器会话结束时被删除。
- **sessionStorage**：
  - 仅在当前会话中有效。关闭窗口或标签页后，数据会被清空。
- **localStorage**：
  - 永久有效，除非主动删除数据或通过浏览器设置清除数据。

#### 3. **存储容量**

- **Cookie**：
  - 每个 Cookie 的大小限制通常为 4KB，总的 Cookie 数量和大小也有限制（通常在 20 个 Cookie，总大小限制约为 4096 字节）。
- **sessionStorage 和 localStorage**：
  - 一般来说，每个域名下的存储限制为 5MB 到 10MB（具体取决于浏览器）。

#### 4. **访问权限**

- **Cookie**：
  - 可以设置为 HTTP-only，防止客户端 JavaScript 访问，主要用于存储会话信息。
- **sessionStorage 和 localStorage**：
  - 可以通过 JavaScript 直接访问，适合存储不需要在服务器端验证的数据。
...
- **Cookie**：
  - 适合存储用户认证信息、跟踪用户会话等。
- **sessionStorage**：
  - 适合临时数据存储，例如用户在单个标签页的输入信息。
- **localStorage**：
  - 适合持久化存储用户偏好设置、应用状态等，不需要频繁更新的数据。
