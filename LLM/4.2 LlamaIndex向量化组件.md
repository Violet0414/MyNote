#### Embeddings(向量化组件)

**核心基类**

```python
from llama_index.core.embeddings import BaseEmbedding

class BaseEmbedding:
    """所有embedding模型的基类"""
    
    def get_text_embedding(self, text: str) -> List[float]:
        """单个文本向量化"""
        
    def get_text_embedding_batch(self, texts: List[str]) -> List[List[float]]:
        """批量文本向量化"""
        
    def get_agg_embedding_from_queries(self, *args) -> List[float]:
        """从查询聚合embedding"""
```



**云端API模型**

```python
# OpenAI
from llama_index.embeddings.openai import OpenAIEmbedding
embed_model = OpenAIEmbedding(
    model="text-embedding-3-small",
    api_key="sk-...",
    embed_batch_size=100  # 批量大小
)

# Azure OpenAI
from llama_index.embeddings.azure_openai import AzureOpenAIEmbedding
embed_model = AzureOpenAIEmbedding(
    model="text-embedding-3-small",
    deployment_name="your-deployment",
    api_key="...",
    azure_endpoint="https://xxx.openai.azure.com/"
)

# Cohere
from llama_index.embeddings.cohere import CohereEmbedding
embed_model = CohereEmbedding(
    cohere_api_key="...",
    model_name="embed-english-v3.0",
    input_type="search_document"
)

# 百度文心
from llama_index.embeddings.baidu_qianfan import QianfanEmbedding
embed_model = QianfanEmbedding(
    model="Embedding-V1",
    qianfan_ak="...",
    qianfan_sk="..."
)
```



**本地模型**

```python
# HuggingFace
from llama_index.embeddings.huggingface import HuggingFaceEmbedding
embed_model = HuggingFaceEmbedding(
    model_name="BAAI/bge-large-zh-v1.5",
    device="cuda",  # 可选：cuda, cpu, mps
    trust_remote_code=True,
    cache_folder="./hf_cache"
)

# Sentence Transformers
from llama_index.embeddings.huggingface import HuggingFaceEmbedding
embed_model = HuggingFaceEmbedding(
    model_name="sentence-transformers/all-mpnet-base-v2",
    normalize_embeddings=True
)

# 自定义本地模型
from llama_index.embeddings.huggingface import HuggingFaceEmbedding
embed_model = HuggingFaceEmbedding(
    model_name="/path/to/your/model",
    tokenizer_name="/path/to/your/tokenizer"
)
```

---



**向量化**

```python
from llama_index.embeddings.huggingface import HuggingFaceEmbedding
import torch

# 使用本地模型路径
embed_model = HuggingFaceEmbedding(
    model_name="./model",  # 本地模型文件夹路径
    device="cuda" if torch.cuda.is_available() else "cpu",  # 自动选择设备
    trust_remote_code=True,  # 如果模型需要自定义代码
    model_kwargs={
        "torch_dtype": torch.float16,  # 半精度节省内存
        "local_files_only": True,      # 强制使用本地文件
    },
    encode_kwargs={
        "normalize_embeddings": True,  # 归一化向量
        "batch_size": 32,
        "show_progress_bar": True
    }
)

# 测试向量化
texts = ["这是一个测试句子", "这是另一个句子"]
embeddings = embed_model.get_text_embedding_batch(texts)

print(f"Embedding维度: {len(embeddings[0])}")
print(f"前5个值: {embeddings[0][:5]}")
```



## 基本交互示例

```python
from llama_index.core import VectorStoreIndex, SimpleDirectoryReader
from llama_index.llms.openai import OpenAI
from llama_index.core import Settings
import os

# 1. 配置 LLM（这里以 OpenAI 为例）
# 设置你的 OpenAI API key
os.environ["OPENAI_API_KEY"] = "your-api-key"

# 创建 LLM 实例
llm = OpenAI(model="gpt-3.5-turbo", temperature=0.7)

# 设置为全局 LLM
Settings.llm = llm

# 2. 加载文档（可选，如果不需要 RAG 可以跳过）
# 从文件夹加载文档
documents = SimpleDirectoryReader("./data").load_data()

# 3. 创建索引
index = VectorStoreIndex.from_documents(documents)

# 4. 创建查询引擎
query_engine = index.as_query_engine()

# 5. 交互式问答
while True:
    # 获取用户输入
    query = input("\n请输入你的问题（输入 'quit' 退出）: ")
    
    if query.lower() in ['quit', 'exit', 'q']:
        print("再见！")
        break
    
    if not query.strip():
        continue
    
    # 执行查询
    try:
        print("\n🤖 正在思考...")
        response = query_engine.query(query)
        print(f"\n💡 回答: {response}")
    except Exception as e:
        print(f"❌ 出错: {e}")
```



## 如果使用本地模型（如 HuggingFace）

```python
from llama_index.core import Settings
from llama_index.llms.huggingface import HuggingFaceLLM
from llama_index.core import PromptTemplate
import torch

# 1. 配置本地 LLM
llm = HuggingFaceLLM(
    model_name="Qwen/Qwen2-7B-Instruct",  # 或使用本地路径 "./model"
    tokenizer_name="Qwen/Qwen2-7B-Instruct",
    context_window=4096,
    max_new_tokens=256,
    generate_kwargs={"temperature": 0.7, "do_sample": True},
    device_map="auto",
)

# 设置为全局 LLM
Settings.llm = llm

# 2. 简单的直接问答（不使用 RAG）
def simple_chat():
    """简单的对话交互"""
    print("🤖 本地模型聊天机器人（输入 'quit' 退出）")
    print("=" * 50)
    
    while True:
        try:
            # 获取用户输入
            user_input = input("\n👤 你: ").strip()
            
            if user_input.lower() in ['quit', 'exit', 'q']:
                print("🤖 再见！")
                break
            
            if not user_input:
                continue
            
            # 构造提示词
            prompt = f"""请回答以下问题：
            
问题：{user_input}

回答："""
            
            # 生成回答
            print("🤖 正在思考...")
            response = llm.complete(prompt)
            
            print(f"🤖 机器人: {response.text}")
            
        except KeyboardInterrupt:
            print("\n🤖 再见！")
            break
        except Exception as e:
            print(f"❌ 出错: {e}")

# 运行聊天
if __name__ == "__main__":
    simple_chat()
```



## 使用本地向量知识的 RAG 问答

```python
from llama_index.core import VectorStoreIndex, SimpleDirectoryReader
from llama_index.llms.huggingface import HuggingFaceLLM
from llama_index.embeddings.huggingface import HuggingFaceEmbedding
from llama_index.core import Settings
import torch

# 1. 配置本地模型
print("🔄 正在加载模型...")

# 配置 LLM
llm = HuggingFaceLLM(
    model_name="Qwen/Qwen2-7B-Instruct",  # 可替换为你的本地模型路径
    tokenizer_name="Qwen/Qwen2-7B-Instruct",
    context_window=4096,
    max_new_tokens=512,
    device_map="auto",
)

# 配置 Embedding 模型
embed_model = HuggingFaceEmbedding(
    model_name="BAAI/bge-small-zh-v1.5",  # 可替换为你的本地 embedding 模型
    device="cuda" if torch.cuda.is_available() else "cpu",
)

# 设置全局配置
Settings.llm = llm
Settings.embed_model = embed_model
Settings.chunk_size = 512
Settings.chunk_overlap = 50

# 2. 加载本地知识库
print("📚 正在加载知识库...")
try:
    # 从 data 文件夹加载文档
    documents = SimpleDirectoryReader("./data").load_data()
    
    # 创建索引
    index = VectorStoreIndex.from_documents(
        documents,
        show_progress=True
    )
    
    # 创建查询引擎
    query_engine = index.as_query_engine(
        similarity_top_k=3,  # 使用前3个最相关的文档
        verbose=True  # 显示详细过程
    )
    
    print("✅ 系统准备就绪！")
    
except Exception as e:
    print(f"❌ 加载知识库失败: {e}")
    print("⚠️  将使用无知识库模式")
    query_engine = None

# 3. 交互式问答
def rag_chat():
    """基于知识库的问答"""
    print("\n" + "="*50)
    print("🤖 智能知识问答系统")
    print("="*50)
    print("提示：")
    print("1. 输入问题获取基于知识库的回答")
    print("2. 输入 'quit' 或 'exit' 退出")
    print("="*50)
    
    while True:
        try:
            # 获取用户问题
            question = input("\n❓ 请输入问题: ").strip()
            
            if question.lower() in ['quit', 'exit', 'q']:
                print("🤖 感谢使用，再见！")
                break
            
            if not question:
                continue
            
            print("\n🔍 正在检索信息...")
            
            if query_engine:
                # 使用知识库回答问题
                response = query_engine.query(question)
                print(f"\n💡 回答: {response}")
                
                # 显示参考来源（如果可用）
                if hasattr(response, 'source_nodes') and response.source_nodes:
                    print("\n📚 参考来源:")
                    for i, node in enumerate(response.source_nodes[:2]):  # 显示前2个来源
                        print(f"{i+1}. {node.text[:150]}...")
            else:
                # 无知识库，直接使用 LLM
                prompt = f"""请回答以下问题：

问题：{question}

回答："""
                response = llm.complete(prompt)
                print(f"\n💡 回答: {response.text}")
                
        except KeyboardInterrupt:
            print("\n🤖 再见！")
            break
        except Exception as e:
            print(f"❌ 出错: {e}")

# 运行
if __name__ == "__main__":
    rag_chat()
```



## 最简单直接的版本（无知识库）

```python
from llama_index.llms.huggingface import HuggingFaceLLM
import torch

# 1. 初始化模型
print("🚀 正在加载模型...")
llm = HuggingFaceLLM(
    model_name="Qwen/Qwen2-7B-Instruct",  # 替换为你的模型路径
    tokenizer_name="Qwen/Qwen2-7B-Instruct",
    device_map="auto",
)

print("✅ 模型加载完成！")

# 2. 简单的问答循环
while True:
    question = input("\n👤 你问: ").strip()
    
    if question.lower() in ['退出', 'quit', 'exit', 'q']:
        print("🤖 再见！")
        break
    
    if not question:
        continue
    
    try:
        print("🤖 正在思考...")
        
        # 直接使用 LLM 生成回答
        response = llm.complete(question)
        
        print(f"🤖 回答: {response.text}")
        
    except Exception as e:
        print(f"❌ 出错: {e}")
```

---